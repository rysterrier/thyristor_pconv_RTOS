...
/**
  ...
  * @param  ADCx: where x can be 1, 2, 3 or 4 to select the ADC peripheral.
  * @param  ADC_InitStruct: pointer to an ADC_InitTypeDef_Std structure that contains
  *         the configuration information for the specified ADC peripheral.
  * @retval None
  */
void ADC_Init(ADC_TypeDef* ADCx, ADC_InitTypeDef_Std* ADC_InitStruct)
{
  uint32_t tmpreg1 = 0;
  /* Check the parameters */
  assert_param(IS_ADC_ALL_PERIPH(ADCx));
...
/**
  ...
  * @param  ADC_InitStruct : pointer to an ADC_InitTypeDef_Std structure which will be initialized.
  * @retval None
  */
void ADC_StructInit(ADC_InitTypeDef_Std* ADC_InitStruct)
{
...
void ADC_TempSensorCmd(ADC_TypeDef* ADCx, FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  ...
  else
   {
     ...
     ADC12_COMMON->CCR &= ~(uint32_t)ADC12_CCR_TSEN;
   }
}
...
void ADC_VrefintCmd(ADC_TypeDef* ADCx, FunctionalState NewState)
{
  ...
  if((ADCx == ADC1) || (ADCx == ADC2))
  {
    ...
    else
    {
      ...
      ADC12_COMMON->CCR &= ~(uint32_t)ADC12_CCR_VREFEN;
    }
  }
  else
  {
    ...
    {
      /* Disable the Vrefint channel*/
      ADC34_COMMON->CCR &= ~(uint32_t)ADC34_CCR_VREFEN;
    }
  }
}
...
void ADC_VbatCmd(ADC_TypeDef* ADCx, FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  ...
  else
   {
     ...
     ADC12_COMMON->CCR &= ~(uint32_t)ADC12_CCR_VBATEN;
   }
}
...
uint32_t ADC_GetDualModeConversionValue(ADC_TypeDef* ADCx)
{
  ...
  if((ADCx == ADC1) || (ADCx== ADC2))
  {
    ...
    tmpreg1 = ADC12_COMMON->CDR;
  }
  else
  {	
    /* Get the dual mode conversion value */
    tmpreg1 = ADC34_COMMON->CDR;
  }
...
FlagStatus ADC_GetCommonFlagStatus(ADC_TypeDef* ADCx, uint32_t ADC_FLAG)
{
  ...
  if((ADCx == ADC1) || (ADCx == ADC2))
  {
    tmpreg1 = ADC12_COMMON->CSR;
  }
  else
  {
    tmpreg1 = ADC34_COMMON->CSR;
  }  
  /* Check the status of the specified ADC flag */
...
void ADC_ClearCommonFlag(ADC_TypeDef* ADCx, uint32_t ADC_FLAG)
{
  ...
  {
    /* Clear the selected ADC flags */
    ADC12_COMMON->CSR |= (uint32_t)ADC_FLAG;
  }
  else
  {
    /* Clear the selected ADC flags */
    ADC34_COMMON->CSR |= (uint32_t)ADC_FLAG;
  }  
}
